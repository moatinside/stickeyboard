...
export default function BoardView() {
  ...
  const { state, pushState, undo, redo, canUndo, canRedo } =
  ...
  useEffect(() => {
    ...
    if (container) {
      container.scrollLeft =
        (CANVAS_SIZE * zoom - container.clientWidth) / 2
      container.scrollTop =
        (CANVAS_SIZE * zoom - container.clientHeight) / 2
  ...
  const handleColorClick = useCallback(
    (color) => {
      ...
      if (activeNoteIdRef.current) {
        ...
        const newNotes = currentState.notes.map((n) =>
          n.id === activeNoteIdRef.current
            ? { ...n, text: editTextRef.current, color }
            : n
        )
        pushState({ ...currentState, notes: newNotes })
      }
  ...
  useEffect(() => {
    ...
    const handleMouseUp = (e) => {
      ...
      if (connectingRef.current) {
        const { x, y } = screenToCanvas(e.clientX, e.clientY)
        const currentState = stateRef.current
        const targetNote = findNoteAtPosition(
          x,
          y,
          currentState.notes,
          connectingRef.current.fromNoteId
        )

        if (targetNote) {
          const exists = currentState.connections.some(
            (c) =>
              c.fromNoteId === connectingRef.current.fromNoteId &&
              c.toNoteId === targetNote.id
          )
          if (!exists) {
            const newConn = {
              id: crypto.randomUUID(),
              fromNoteId: connectingRef.current.fromNoteId,
              toNoteId: targetNote.id,
            }
            pushState({
              ...currentState,
              connections: [...currentState.connections, newConn],
            })
          }
        }
        setConnecting(null)
  ...
  const getNotePosition = (note) => {
    if (dragOffset && dragOffset.noteId === note.id) {
      return {
        x: Math.max(0, note.x + dragOffset.dx),
        y: Math.max(0, note.y + dragOffset.dy),
      }
    }
    return { x: note.x, y: note.y }
  }
  ...
  return (
    <div className="board-view">
          ...
          >
            ...
            >
              ...
              {state.connections.map((conn) => {
                const fromNote = state.notes.find(
                  (n) => n.id === conn.fromNoteId
                )
                const toNote = state.notes.find(
                  (n) => n.id === conn.toNoteId
                )
                if (!fromNote || !toNote) return null

                const fPos = getNotePosition(fromNote)
                const tPos = getNotePosition(toNote)
                const { from, to } = getConnectionEndpoints(
                  { ...fromNote, ...fPos },
                  { ...toNote, ...tPos }
                )
                ...
                >
                  ...
                  <div className="connection-points">
                    {['top', 'right', 'bottom', 'left'].map((side) => (
                      <div
                        key={side}
                        className={`conn-point conn-${side}`}
                        onMouseDown={(e) =>
                          handleConnStart(e, note.id, side)
                        }
                      />
                    ))}
                  </div>
      ...
      <div className="board-footer">
        ダブルクリックで付箋を追加 ｜ ホイールでズーム ｜ Ctrl+Z で元に戻す
      </div>
    </div>
  )
}
